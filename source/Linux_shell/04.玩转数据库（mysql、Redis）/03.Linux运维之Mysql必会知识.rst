...
| ​
| d、模糊查询 INSERT INTO students2 VALUES(0,‘习近平’,65,1,‘北京’,0);
  INSERT INTO students2 VALUES(0,‘习大大’,66,1,‘北京’,0); INSERT INTO
  students2 VALUES(0,‘习大’,64,1,‘北京’,0); like % 表示任意多个任意字符
  \_表示一个任意字符
...
| ​
| f、空判断 注意：null与""是不同 判断空：is null 判断非空：is not null
...
| 4.聚合 为了快速等到统计数据，提供5个聚合函数 a、count(\ *)
  ...
  c、min(列) 表示求此列的最小值 d、sum(列) 表示求此列的和 e、avg(列)
  表示求此列的平均值
...
| ​
| 分组后的数据筛选： select 列1,列2,聚合…… from 表名 group by 列1, 列2,
  列3,….. having 列1,列2…..,聚合…. 示例：select gender,count(\ *) from
  students2 group by gender having gender; select gender,count(*) from
  students2 group by gender having gender=0;
...
| 2、 CREATE TABLE ``students3`` ( ``id`` int(11) NOT NULL
  ...
  DEFAULT b’1’, ``classid`` int(11) NOT NULL, PRIMARY KEY (``id``), KEY
  ``classid`` (``classid``), CONSTRAINT ``students3_ibfk_1`` FOREIGN KEY
...
.. |image0| image:: ../../_static/mysql-insert.PNG
.. |image1| image:: ../../_static/mysql-data.png
.. |image2| image:: ../../_static/mysql-data-time.png
.. |image3| image:: ../../_static/string.png
.. |image4| image:: ../../_static/mysql-yingqing.png
.. |image5| image:: ../../_static/mysql_slow001.png
