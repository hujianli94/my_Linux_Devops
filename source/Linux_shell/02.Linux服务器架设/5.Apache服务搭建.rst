...
Worker 模式下所能同时处理的请求总数 是由子进程总 数乘以 ThreadsPerChild
值决定的,应该大于等于 MaxClients.如果负载很大,现有的子进程
数不能满足时,控制进程会派生新的子进程.默认最大的子进程总数是
16,加大时也需要显式 声明 ServerLimit（最大值是 20000）
需要注意的是,如果显式声明了 ServerLimit,那么它乘以 ThreadsPerChild
的值必须大于等 于 MaxClients,而且 MaxClients 必须是 ThreadsPerChild
的整数倍,否则 Apache 将会自 动调节到一个相应值（可能是个非期望值）.
...
-  这段规则的含义是允许使用Firefox浏览器的主机访问服务器上的首页文件，除此之外的所有请求都将被拒绝。
   ...
   #重启httpd服务 [root@host-10-144-114-67 admin]# systemctl restart
   httpd
...
.. |image0| image:: ../../_static/apache1.PNG
.. |image1| image:: ../../_static/apache2.PNG
.. |image2| image:: ../../_static/apache4.PNG
.. |image3| image:: ../../_static/apache5.PNG
.. |image4| image:: ../../_static/apache6.PNG
.. |image5| image:: ../../_static/apache7.PNG
.. |image6| image:: ../../_static/apache8.PNG
