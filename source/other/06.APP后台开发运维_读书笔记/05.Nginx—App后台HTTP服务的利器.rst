...
Nginx是由Igor
Sysoev为俄罗斯访问量第二的Rambler.ru站点开发的，其已经在该站点运行超过三年。Igor将Nginx源代码以类似BSD许可证的形式发布。
...
正常工作的Nginx有多个进程，基本的有master process（称为主进程）和worker
process （称为工作进程）。
...
Nginx中master和worker的工作流程 |image0|
...
设置sendfile为off时传输文件的流程： |image2|

设置sendfile为on时传输文件的流程: |image3|
...
如果需要下载时文件名后缀就是.apk或.ipa，可以修改Nginx
conf目录下的mime.types文件,在文件中增加下面的两行。
...
把上面的代码加入到localhost的配置文件中，在浏览器输入网址http://localhost/lua，就能看到浏览器中输出字符串“hello,
world.”。
...
.. |image0| image:: ../../_static/nginx_jinchengmanager01.png
.. |image1| image:: ../../_static/nginx_config.png
.. |image2| image:: ../../_static/nginx_config002.png
.. |image3| image:: ../../_static/nginx_config003.png
.. |image4| image:: ../../_static/nginx_loadbance01.png
