...
针对某个目录做用户认证，如针对admin目录做用户认证。只需要把bone.conf.conf配置文件中的`location /` 改为`location /admin/`即可。
...
Nginx主配置文件`nginx.conf`中有一段代码是 log_format,log_format是定义Nginx访问日志的格式，代码和格式含义如下：
...
__所以需要给每个站点都设置一个pool，把这些站点隔离开__
...
[php-fpm的pool、php-fpm慢执行日志、open_basedir、php-fpm进程管理](https://www.cnblogs.com/lms0755/p/9179173.html)
